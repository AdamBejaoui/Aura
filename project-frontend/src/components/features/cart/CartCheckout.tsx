  ...
  return (
    <div className="fixed inset-0 z-[70] flex justify-end">
      ...
      <motion.div
        ...
        className="relative w-full max-w-md md:max-w-5xl bg-white dark:bg-neutral-900 h-full shadow-[0_0_50px_rgba(0,0,0,0.1)] flex flex-col border-l border-stone-100 dark:border-neutral-800 md:rounded-l-[3rem] p-8 md:p-10 pt-safe overflow-hidden"
      >
        {/* Header */}
        <div className="flex items-center justify-between mb-12 bg-transparent z-20">
        ...
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden -mx-8 md:-mx-10">
          <AnimatePresence mode="wait">
            {isSuccess ? (
              ...
              <>
                {/* Order Summary */}
                <div className="w-full md:w-[50%] bg-stone-50 dark:bg-neutral-800/50 p-6 md:p-10 pt-0 overflow-y-auto no-scrollbar border-b md:border-b-0 md:border-r border-stone-100 dark:border-neutral-800">
                  <div className="mb-8">
                  ...
                  <div className="space-y-6">
                    {items.map((item) => (
                      <div key={item.product.id} className="flex gap-4 group">
                        <div className="w-16 h-20 bg-white dark:bg-neutral-800 rounded-2xl overflow-hidden border border-stone-100 dark:border-neutral-700 flex-shrink-0">
                        ...
                        <div className="flex-1 flex flex-col justify-between py-1">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2 bg-white dark:bg-neutral-800 rounded-xl px-2.5 py-1 border border-stone-100 dark:border-neutral-700">
                              <button onClick={() => onUpdateQuantity(item.product.id, Math.max(0, item.quantity - 1))} className="text-stone-300 hover:text-stone-900 dark:hover:text-white text-xs">-</button>
                              <span className="text-[10px] font-black dark:text-white w-4 text-center">{item.quantity}</span>
                              <button onClick={() => onUpdateQuantity(item.product.id, item.quantity + 1)} className="text-stone-300 hover:text-stone-900 dark:hover:text-white text-xs">+</button>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-stone-200 dark:border-stone-800">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xl font-black text-stone-900 dark:text-white tracking-widest">${orderTotal.toLocaleString()}</span>
                    </div>
                  </div>
                  <div className="h-20 lg:hidden" aria-hidden="true" />
                </div>

                <div className="flex-1 p-6 md:p-10 pt-0 bg-white dark:bg-neutral-900 overflow-y-auto no-scrollbar">
                  <div className="mb-8">
                  ...
                  <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <textarea value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Full Address" required rows={2} className="w-full bg-stone-50 dark:bg-neutral-800 border border-stone-100 dark:border-neutral-800 rounded-2xl pl-12 pr-6 py-3.5 text-xs font-black focus:outline-none focus:ring-4 focus:ring-stone-900/5 transition-all dark:text-white resize-none" />
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-5 gap-2">
                        {sizes.map((s) => (
                          <button key={s} type="button" onClick={() => setSize(s)} className={`py-2.5 rounded-xl text-[10px] font-black transition-all border ${size === s ? 'bg-stone-900 dark:bg-white text-white dark:text-black border-transparent shadow-xl' : 'bg-stone-50 dark:bg-neutral-800 text-stone-400 border-stone-100 dark:border-neutral-800'}`}>{s}</button>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <button type="button" onClick={() => setPaymentMethod('cod')} className={`flex items-center gap-3 p-3.5 rounded-2xl border transition-all ${paymentMethod === 'cod' ? 'bg-stone-50 dark:bg-neutral-800/50 border-stone-900 dark:border-white' : 'bg-white dark:bg-neutral-900 border-stone-100 dark:border-neutral-800'}`}>
                        ...
                        <button type="button" disabled className="flex items-center gap-3 p-3.5 rounded-2xl border border-stone-100 dark:border-neutral-800 bg-stone-50/50 dark:bg-neutral-900/50 opacity-50 cursor-not-allowed">
