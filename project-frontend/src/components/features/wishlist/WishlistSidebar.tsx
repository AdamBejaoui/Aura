                ...
                <div className="flex items-center justify-between mb-12">
                    <div className="flex items-center gap-3">
                        <div className="p-2.5 bg-stone-50 dark:bg-neutral-800 rounded-2xl border border-stone-100 dark:border-neutral-800">
                            <Heart className="w-5 h-5 text-stone-900 dark:text-white" />
                        </div>
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar">
                    {items.length === 0 ? (
                        ...
                        <div className="space-y-8">
                            {items.map((product) => (
                                <div key={product.id} className="group relative flex gap-6 p-4 rounded-[2rem] hover:bg-stone-50 dark:hover:bg-neutral-800/50 transition-all border border-transparent hover:border-stone-100 dark:hover:border-neutral-800/50">
                                    <div className="relative w-28 h-36 bg-stone-50 dark:bg-neutral-800 rounded-2xl overflow-hidden flex-shrink-0 border border-stone-100 dark:border-neutral-800">
                                        <img
                                    ...
                                    <div className="flex-1 flex flex-col justify-between py-2">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <span className="text-lg font-black text-stone-900 dark:text-white tracking-tighter">
                                                {formatCurrency(product.price, product.currency)}
                                            </span>
                                            {product.inStock ? (
                                                <button
                                                    onClick={() => handleAddToCart(product)}
                                                    className="p-2.5 bg-stone-900 dark:bg-white text-white dark:text-black rounded-xl hover:shadow-xl active:scale-90 transition-all"
                                                >
                                                    <ShoppingBag className="w-4 h-4" />
                                                </button>
                                            ) : (
                                                <span className="text-[8px] font-black text-red-500 bg-red-50 dark:bg-red-900/20 px-3 py-1.5 rounded-lg uppercase">
                                                    Sold Out
                                                </span>
                                            )}
                                        </div>
