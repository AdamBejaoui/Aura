...
const ProductDetailModal = ({
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-5xl max-h-[95vh] overflow-hidden rounded-2xl sm:rounded-3xl bg-white dark:bg-neutral-900 shadow-2xl ring-1 ring-gray-200 dark:ring-neutral-800 transition-all flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute right-3 top-3 sm:right-4 sm:top-4 z-20 flex h-9 w-9 sm:h-10 sm:w-10 items-center justify-center rounded-full bg-white/10 backdrop-blur-md text-white md:text-gray-500 md:bg-white md:shadow-sm md:hover:bg-gray-100 dark:md:bg-neutral-800 dark:md:text-gray-400 transition-all hover:scale-105"
        >
          <X className="h-5 w-5" />
        </button>

        <div className="grid gap-0 md:grid-cols-2 flex-1 overflow-y-auto md:overflow-hidden">
          {/* Image Gallery */}
          ...
          {/* Details Section */}
          <div className="flex flex-col justify-between p-6 sm:p-8 md:p-10 bg-white dark:bg-neutral-900 min-h-0">
            ...
            <div className="mt-6 sm:mt-8 pt-4 flex-shrink-0">
              <button
                onClick={() => {
                  onAddToCart?.(product);
                  onClose();
                }}
                className="group relative flex w-full items-center justify-center gap-3 overflow-hidden rounded-xl bg-stone-900 dark:bg-white px-6 sm:px-8 py-3 sm:py-4 text-white dark:text-black transition-all hover:bg-black dark:hover:bg-gray-200 hover:shadow-lg hover:shadow-stone-900/25 dark:hover:shadow-white/10 active:scale-[0.98]"
              >
                <ShoppingBag className="h-5 w-5 transition-transform group-hover:-translate-y-1 group-hover:rotate-12" />
                <span className="font-bold text-base sm:text-lg">Add to Cart</span>
              </button>
            </div>
          </div>
        </div>
